...
### 2. **Database Setup**
Create a MySQL database (e.g., `vyking`) and import the necessary schema and stored procedures.
...
### 2.4.1 **Add stored procedure to distribute prizes**

Procedure to distribute prizes based on tournament ID.
Prizes are distributed to the top 3 players based on their total bets in the tournament.
Edge cases are handled, such as when there are less than 3 players in the tournament or when players have the same total bets.
...
### 2.4.2. **Test the stored procedure**

```sql

-- Test the stored procedure by distributing prizes for tournament with ID 1
CALL distribute_prizes(1);
